...
non-uniform but non-collapsed token-credit signal, the empirical observable
predicted by the collapse analysis.}
...
between 0.052 and 0.073 on average. ARCA occupies the predicted middle regime:
it is far from uniform, but it does not collapse onto the tiny effective
...
is not secondary. It is the observable predicted by the LoRA-collapse
analysis: a method whose salience is measured through output distributions
either broadcasts credit uniformly or concentrates it on a very small set of
...
evaluated only by final task accuracy. Accuracy alone cannot tell whether a
method actually changed token-level credit or merely produced a different
optimization trajectory. The concentration diagnostics separate these cases.
ARCA demonstrates that one can obtain an adaptation-aware credit signal from
the LoRA adapter itself, with no value head, process reward model, or tree
