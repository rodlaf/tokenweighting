...
\cite{yu2026erpo}. \emph{Sparse but Critical} argues, via an advantage-concentration
analysis, that a small fraction of tokens accounts for most of the useful
update-magnitude in RLVR, and proposes a divergence-based selection rule for
identifying them~\cite{meng2026sparsebutcritical}. These entropy- and divergence-based
...
Reinforced Token Optimization (RTO) frames RLHF as a token-level MDP and derives an
optimal Q-function-based policy from the preference model, producing per-token credit
without a separate value head \cite{zhong2025rto}. These methods can produce rich
...
advantage at each position~\cite{li2026otb}. PSPO applies potential-based reward
...
On the analysis side, \emph{Narrow Finetuning Traces} studies how LoRA's rank-$r$
updates appear in hidden states and gradients, finding that the adapter residual is
a highly informative trace of what the adapter has
learned~\cite{minder2025narrowfinetuning}. This is directly relevant to ARTW, because it
supplies empirical evidence that the adapter residual is both position-varying and
semantically meaningful---exactly the property our method exploits. Concurrent work on
TopLoRA/GateRA studies how to concentrate LoRA capacity on a small number of
high-impact tokens via input-dependent projections~\cite{li2025toplora}. This can be
...
\cite{huang2024red,cui2025prime,tran2025tempo,cao2025scar,parthasarathi2025grpolambda,li2026otb,hu2026pspo,he2026eapo,yu2026erpo,meng2026sparsebutcritical}.
