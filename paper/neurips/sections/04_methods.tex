...
log-probability ratios are compressed toward zero~\citep{implicitklra2025}.
...
\subsection{Adapter-Residual Token Weighting (ARTW)}\label{sec:artw}
...
Define the Adapter-Residual Token Weighting salience as
\begin{equation}
\alpha_t^{\mathrm{ARTW}}(x, y) = \lVert h_t^{\mathrm{adapted}} -
h_t^{\mathrm{base}} \rVert_2,
\end{equation}
and normalize within the trajectory as in
equation~\eqref{eq:normalize} to obtain per-token weights
$w_t^{\mathrm{ARTW}}$. The adapter residual is computed once per minibatch
...
reference policy or to support divergence weighting, so ARTW adds no new
infrastructure.

\paragraph{Why ARTW does not collapse.}
The key property that distinguishes ARTW from the intrinsic schemes above is
...
the signal $\alpha_t^{\mathrm{ARTW}}$ vanishes uniformly, but its
\emph{normalized} counterpart $w_t^{\mathrm{ARTW}}$ remains non-degenerate
...
ARTW can be read as a cheap proxy for a gradient-based notion of token
...
Critical}~\cite{sparsebutcritical2026}, entropy-based weighting
to~\cite{wang2026eapo,chen2026erpo}). ARTW is the only scheme we
study that uses an explicitly \emph{adaptation-aware} signal. Unlike
critic-based PPO, ARTW does not learn a value head. Unlike RED, PRIME, or
token-level reward-model methods, ARTW does not construct dense rewards from
an auxiliary model. Unlike TEMPO or related tree-based methods, ARTW does not
build prefix graphs or estimate branch values. Unlike hard token-selection
methods based on top-entropy masking, ARTW retains a dense token update but
