...
trainable, while practical LLM-RL pipelines overwhelmingly rely on
...
restricted to a low-rank neighbourhood of the reference model, so the
per-token output-distribution differences used by common intrinsic credit
signals---surprisal, entropy reduction, and policy divergence---can be driven
toward degenerate normalized weights. We formalize this collapse and propose
measuring it directly with concentration diagnostics such as weight Gini and
effective-token ratio. We then introduce \emph{Adapter-Residual Credit
...
uncertain or shifted, and requires no reward model, value head, or tree
construction. In a compact MATH/Qwen3-1.7B GRPO sweep, ARCA exhibits the
predicted non-collapsed middle-regime credit distribution under matched
