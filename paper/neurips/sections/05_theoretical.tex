...
\subsection{Signal Collapse Under Low-Rank Adaptation}\label{sec:theory-collapse}

The preceding analysis assumes that the intrinsic weights
$\{w_t\}_{t=1}^T$ are non-degenerate: if every scheme degenerates to
$w_t \equiv 1/T$, then token weighting cannot provide any credit
redistribution over uniform. We now formalize when this degeneracy occurs
under LoRA.
...
the normalized effective-token count.

\begin{proposition}[Collapse of surprisal and entropy weighting]
...
\]
In particular, if the base-model surprisal profile is close to uniform (as it
is on content-dense reasoning traces with smooth base-model calibration),
$w^{\mathrm{surp}} \to 1/T$ as $\beta / \min_t \alpha_t^{\mathrm{surp,ref}}
\to 0$. The analogous statement holds for the entropy-reduction score.
\end{proposition}
...
\begin{proposition}[Collapse of divergence weighting]
...
$w^{\mathrm{div}} = \alpha^{\mathrm{div}} / \sum_t \alpha_t^{\mathrm{div}}$.
Then
\[
0 \le \alpha_t^{\mathrm{div}} \le 2 L \beta \qquad \forall t,
\]
so $\max_t \alpha_t^{\mathrm{div}} / \min_t (\alpha_t^{\mathrm{div}} +
\varepsilon) \to 1$ as $\beta \to 0$, and consequently $G(w^{\mathrm{div}})
\to 0$ and $\mathrm{EffN}(w^{\mathrm{div}})/T \to 1$. That is, divergence
weighting collapses to the uniform distribution whenever the LoRA adapter's
per-token perturbation norm vanishes uniformly.
\end{proposition}
...
The collapse is not a property of a particular weighting scheme; it is a
property of measuring per-token variation through the output distribution
when the policy has been constrained to a small neighbourhood of the
reference.

\subsection{ARTW and Position-Discriminative Signal}\label{sec:theory-artw}

ARTW side-steps the collapse by measuring a quantity that is position-varying
even when the adapter's logit-space impact is small.

\begin{proposition}[ARTW remains non-degenerate]
\label{prop:artw-nondeg}
...
\[
\mathrm{Var}_t(\alpha_t^{\mathrm{ARTW}}) \ge \max_\ell V_\ell
...
any layer, $\alpha_t^{\mathrm{ARTW}}$ has bounded-below variance across
positions, and its normalized weights $w_t^{\mathrm{ARTW}}$ do not converge
to the uniform distribution as the adapter shrinks uniformly.
\end{proposition}

The intuitive content of Proposition~\ref{prop:artw-nondeg} is that ARTW
...
the adapter weights uniformly scales $\alpha^{\mathrm{ARTW}}$ uniformly, so
the normalized weights $w^{\mathrm{ARTW}}$ are scale-invariant in the
adapter magnitude.
...
most published weighting methods rely on are driven toward uniform; the
adapter-residual signal that ARTW uses is, by Proposition~\ref{prop:artw-nondeg},
provably non-degenerate in the same regime.
