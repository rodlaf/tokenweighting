...
\cite{chen2026erpo}. \emph{Sparse but Critical} argues, via an advantage-concentration
...
identifying them~\cite{sparsebutcritical2026}. These entropy- and divergence-based
...
and GenAC~\cite{genac2026} cover non-parametric and generative critics
...
the optimal policy \cite{zhang2026pspo}. Compared with these approaches, our focus is
...
the default adaptation strategy for essentially all open-source LLM-RL work. TinyLoRA
showed that small, aggressively parameter-efficient adapters can match full fine-tuning
on RLHF benchmarks~\cite{tinylora2026}. Tina demonstrated the same for RLVR reasoning
tasks, obtaining DeepSeek-R1-class reasoning behaviour on significantly smaller budgets
by combining LoRA with GRPO~\cite{tina2025}. A broader evaluation of PEFT for RLVR
systematically compared LoRA with full fine-tuning across tasks and scales, finding
that LoRA is competitive or preferable once regularisation and data scale are
controlled~\cite{peftrlvr2025}. Token-Efficient RL explicitly optimises the number of
tokens required to achieve a given RLVR accuracy level and reports that full-token
GRPO~+~LoRA can fail to train reliably on some
reasoning tasks, an empirical observation for which we now provide a mechanistic
explanation via signal collapse~\cite{tokenefficientrl2025}. \emph{LoRA as Implicit KL
Regulariser} analyses how LoRA restricts the policy to a rank-constrained neighbourhood
of the reference, arguing that this restriction implicitly bounds the KL divergence
between policy and reference throughout training~\cite{implicitklra2025}. Our Section
...
learned~\cite{narrowfinetuning2026}. This is directly relevant to ARTW, because it
...
high-impact tokens via gating~\cite{toplora2026}. This can be viewed as a complementary
response to the same underlying problem: TopLoRA modifies the adapter itself so that
...
\cite{huang2024red,cui2025prime,tran2025tempo,cao2025scar,parthasarathi2025grpolambda,li2026otb,zhang2026pspo,wang2026eapo,chen2026erpo,sparsebutcritical2026}.
...
regime~\cite{hu2021lora,tinylora2026,tina2025,peftrlvr2025,implicitklra2025,tokenefficientrl2025,narrowfinetuning2026,toplora2026}.
